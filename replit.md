...
- **B2B Wholesale Platform**: Customer and order management with dedicated authentication, category-specific pricing, multi-location support, and advanced features like "Where to Buy" visibility. Admin dashboard for customer, order, tier, and sales rep management. **Sales Order Workflow**: Email-based order processing with secure token links (7-day expiration):
  1. Order Created → Sales rep receives email with link to set delivery date
  2. Delivery Date Set → Admins receive email with invoice preview and approval link
  3. Order Approved → Customer receives invoice email with delivery date; status changes to "approved_delivery_pending"
  4. Delivery Confirmed → Status changes to "delivered_pending_payment"
  5. Payment Recorded → Order marked as "completed", commissions marked as "earned"
  - Each order generates one unique invoice number (INV-timestamp format)
  - Token-based workflow pages: `/b2b/order-delivery/:token`, `/b2b/order-approval/:token`, `/b2b/order-confirm-delivery/:token`
  - Schema fields: deliveryDateToken, approvalToken, deliveryConfirmationToken, scheduledDeliveryDate, invoiceNumber, approvedAt, deliveredAt, paidAt
