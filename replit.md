...
- **Landing Page**: Features tasting app-style aesthetic with winery aerial background image and gradient overlay. Access code WHOLESALE2025 grants view of wholesale pricing tiers without requiring account creation. Database-driven slideshow system with admin management.
- **B2B Slideshow System**: Dynamic slideshow on landing page with database-backed content management. Admins can create/edit/delete slides via the Slideshow tab in the B2B Admin Dashboard. Supports 2-column layout (1/3 media, 2/3 content), image/video media, icons (Sprout, Users, Award, Wine, Package, TrendingDown, Shield, Heart, Star), active/inactive states, and custom ordering. Public endpoint (`/api/b2b/slideshow/slides`) returns only active slides ordered by sort_order.
