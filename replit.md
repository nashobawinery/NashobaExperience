...
- **Media Center Module** (`/media-center`): Multi-channel digital signage management system with sections for NashobaTV (digital signage with various slide types, including Trivia and Historical Facts), Toast Menu Printer, Live Music (musician management, schedule, public submission form), Special Events (event management with Shopify ticket links), Flyer Printer, Shelf Talkers (printable product tags), and **Flight Cards** (printable tasting flight cards pulled from the internal product catalog — multi-category filtering, ordered product selection 1–8+, 3 templates: Classic Winery/Modern Clean/Rustic Craft, 5 paper sizes: A6/4×6/A5/5×7/Half Sheet, field toggles, header/footer, save/load named configs, Staff Print Board toggle; server renders via `/api/media/flight-cards/print`, configs stored in `flight_card_configs` table).
