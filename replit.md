...
- **NashobaTV Digital Signage** (`/display`): Full-screen TV display system with auto-rotating slides optimized for venue TV screens. Slide types include: Welcome, Today's Events, Wine List, Craft Beverages, Upcoming Events, Photo Gallery, Announcements, Wine Club Promo, Daily Specials, and Custom slides. Admin management integrated into Tasting Admin Dashboard under "Digital Signage > NashobaTV" tab with sub-tabs for Settings, Slides, Events, Announcements, Photos, and Specials. Public display at `/display` requires no authentication. Database tables: `nashobatv_slides`, `nashobatv_events`, `nashobatv_announcements`, `nashobatv_photos`, `nashobatv_display_settings`, `nashobatv_daily_specials`. Routes in `server/nashobatv-routes.ts`, display page in `client/src/pages/NashobatvDisplay.tsx`, admin in `client/src/components/NashobatvAdmin.tsx`.
