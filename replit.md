...
- **Data & Marketing Command Center** (unified module at `/command-center`, merging former RCC and Boomerang): Unified hub combining revenue tracking, customer insights, marketing campaigns, and AI-powered recommendations. Left sidebar navigation with expandable sections: Dashboard (combined KPIs), Revenue (Weekly Focus, Daily Revenue, Tasks, Revenue Campaigns), Customers (Segments Overview, Customer Browser, High Value Targets, RFM Analysis), Marketing (Campaigns, Automations, Loyalty Program, Referral Program), Insights (Analytics, AI Advisor), Settings (Integrations, Documentation). Old routes `/rcc` and `/boomerang` redirect to `/command-center`. Components are exported from `client/src/pages/rcc/RccDashboard.tsx` and `client/src/pages/reactivation/BoomerangDashboard.tsx`, composed in `client/src/pages/command-center/CommandCenter.tsx`.
  ...
  - **Growth Studio** (Zeely-inspired): AI-powered marketing toolkit within Command Center sidebar. Five features: (1) AI Content Studio - generates social posts, email subjects, ad copy, event promos, SMS blasts with 4 variations per generation using OpenAI; (2) Content Calendar - monthly visual calendar for planning marketing content by channel with add/edit/delete entries; (3) Campaign Builder - guided 3-step wizard for creating campaigns (name/goal, segment/channels, AI strategy generation); (4) Marketing Scorecard - AI-generated performance assessment with overall score, channel scores, metrics, insights, and recommendations; (5) Quick Promotions - one-click content generation from 6 templates (seasonal, new release, weather deal, event, loyalty, flash sale). Tables: `cc_content_assets`, `cc_content_calendar`, `cc_campaign_builder`, `cc_marketing_scorecards`, `cc_quick_promotions`. Routes in `server/growth-studio-routes.ts`. UI in `client/src/pages/command-center/AiContentStudio.tsx`, `ContentCalendar.tsx`, `CampaignBuilder.tsx`, `MarketingScorecard.tsx`, `QuickPromotions.tsx`. APIs: `/api/growth-studio/*`.
  - **APIs**: `/api/rcc/*`, `/api/reactivation/*`, `/api/boomerang/*`, `/api/toast/*`, `/api/shopify/*`, `/api/targeting/*`, `/api/sms/*`, `/api/growth-studio/*`.
