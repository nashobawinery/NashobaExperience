...
async function runImport() {
  ...
  for (const sql of statements) {
    try {
      ...
      success++;
      const name = sql.match(/VALUES \('[^']+', '([^']+)'/)?.[1] || 'record';
  ...
  // Verify counts
  const expCount = await client.query('SELECT COUNT(*) FROM resy_experiences');
  const mpCount = await client.query('SELECT COUNT(*) FROM resy_meal_periods');
  const ssCount = await client.query('SELECT COUNT(*) FROM resy_site_settings');
  
  console.log(`\n=== RESULTS ===`);
  console.log(`Inserted: ${success}, Skipped/Errors: ${errors}`);
  console.log(`Production counts:`);
  console.log(`  Experiences: ${expCount.rows[0].count}`);
  console.log(`  Meal Periods: ${mpCount.rows[0].count}`);
  console.log(`  Site Settings: ${ssCount.rows[0].count}`);
