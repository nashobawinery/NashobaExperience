...
export const lmsCourses = pgTable("lms_courses", {
  ...
  name: text("name").notNull(),
  description: text("description"),
  category: text("category").notNull(), // training, safety, compliance, orientation, skills, regulatory
  ...
  regulatoryReference: text("regulatory_reference"), // e.g., "OSHA 1910.1200"
  ...
  imageUrl: text("image_url"), // Course thumbnail image
...
export const insertLmsCourseSchema = createInsertSchema(lmsCourses).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
// Course assignments - Track which users are assigned to which courses
export const lmsCourseAssignments = pgTable("lms_course_assignments", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: varchar("user_id").references(() => platformUsers.id).notNull(),
  courseId: varchar("course_id").references(() => lmsCourses.id, { onDelete: "cascade" }).notNull(),
  
  // Assignment details
  assignedById: varchar("assigned_by_id").references(() => platformUsers.id),
  ...
  index("idx_lms_assignments_user").on(table.userId),
  index("idx_lms_assignments_course").on(table.courseId),
  index("idx_lms_assignments_status").on(table.status),
]);

export const insertLmsCourseAssignmentSchema = createInsertSchema(lmsCourseAssignments).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const lmsCourseLessons = pgTable("lms_course_lessons", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  courseId: varchar("course_id").references(() => lmsCourses.id, { onDelete: "cascade" }).notNull(),
...
export const insertLmsCourseLessonSchema = createInsertSchema(lmsCourseLessons).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
export type InsertLmsCourseLesson = z.infer<typeof insertLmsCourseLessonSchema>;
export type LmsCourseLesson = typeof lmsCourseLessons.$inferSelect;

// Lesson Pages - sub-sections within a lesson
export const lmsLessonPages = pgTable("lms_lesson_pages", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  lessonId: varchar("lesson_id").references(() => lmsCourseLessons.id, { onDelete: "cascade" }).notNull(),
...
export const insertLmsLessonPageSchema = createInsertSchema(lmsLessonPages).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const lmsLessonContentBlocks = pgTable("lms_lesson_content_blocks", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  lessonId: varchar("lesson_id").references(() => lmsCourseLessons.id, { onDelete: "cascade" }).notNull(),
  pageId: varchar("page_id").references(() => lmsLessonPages.id, { onDelete: "cascade" }),
  
  // Block type: text, video, image, file, embed, divider
  blockType: text("block_type").notNull(),
  
  // Content based on type
  content: text("content"), // For text blocks - HTML/rich text content
  videoUrl: text("video_url"), // For video blocks
  imageUrl: text("image_url"), // For image blocks
  fileUrl: text("file_url"), // For file blocks
  fileName: text("file_name"),
...
export const insertLmsLessonContentBlockSchema = createInsertSchema(lmsLessonContentBlocks).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
export type InsertLmsLessonContentBlock = z.infer<typeof insertLmsLessonContentBlockSchema>;
export type LmsLessonContentBlock = typeof lmsLessonContentBlocks.$inferSelect;
...
export const insertLmsQuestionBankSchema = createInsertSchema(lmsQuestionBanks).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertLmsQuestionSchema = createInsertSchema(lmsQuestions).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const lmsQuizzes = pgTable("lms_quizzes", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  courseId: varchar("course_id").references(() => lmsCourses.id, { onDelete: "cascade" }).notNull(),
  lessonId: varchar("lesson_id").references(() => lmsCourseLessons.id, { onDelete: "cascade" }),
...
export const insertLmsQuizSchema = createInsertSchema(lmsQuizzes).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const lmsQuizQuestions = pgTable("lms_quiz_questions", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  quizId: varchar("quiz_id").references(() => lmsQuizzes.id, { onDelete: "cascade" }).notNull(),
  questionId: varchar("question_id").references(() => lmsQuestions.id, { onDelete: "cascade" }).notNull(),
...
export const insertLmsQuizQuestionSchema = createInsertSchema(lmsQuizQuestions).omit({
  id: true,
  createdAt: true,
});
export type InsertLmsQuizQuestion = z.infer<typeof insertLmsQuizQuestionSchema>;
...
export const lmsQuizAttempts = pgTable("lms_quiz_attempts", {
  ...
  quizId: varchar("quiz_id").references(() => lmsQuizzes.id, { onDelete: "cascade" }).notNull(),
  userId: varchar("user_id").references(() => platformUsers.id).notNull(),
  assignmentId: varchar("assignment_id").references(() => lmsCourseAssignments.id),
  ...
  needsManualGrading: boolean("needs_manual_grading").notNull().default(false),
  gradedById: varchar("graded_by_id").references(() => platformUsers.id),
  ...
  index("idx_lms_quiz_attempts_quiz").on(table.quizId),
  index("idx_lms_quiz_attempts_user").on(table.userId),
]);

export const insertLmsQuizAttemptSchema = createInsertSchema(lmsQuizAttempts).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const lmsQuestionResponses = pgTable("lms_question_responses", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  attemptId: varchar("attempt_id").references(() => lmsQuizAttempts.id, { onDelete: "cascade" }).notNull(),
  questionId: varchar("question_id").references(() => lmsQuestions.id).notNull(),
...
export const insertLmsQuestionResponseSchema = createInsertSchema(lmsQuestionResponses).omit({
  id: true,
});
export type InsertLmsQuestionResponse = z.infer<typeof insertLmsQuestionResponseSchema>;
export type LmsQuestionResponse = typeof lmsQuestionResponses.$inferSelect;
...
export const lmsLessonProgress = pgTable("lms_lesson_progress", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  assignmentId: varchar("assignment_id").references(() => lmsCourseAssignments.id, { onDelete: "cascade" }).notNull(),
  lessonId: varchar("lesson_id").references(() => lmsCourseLessons.id, { onDelete: "cascade" }).notNull(),
  userId: varchar("user_id").references(() => platformUsers.id).notNull(),
  ...
  lastContentBlockId: varchar("last_content_block_id").references(() => lmsLessonContentBlocks.id),
  ...
  index("idx_lms_lesson_progress_assignment").on(table.assignmentId),
  index("idx_lms_lesson_progress_user").on(table.userId),
]);

export const insertLmsLessonProgressSchema = createInsertSchema(lmsLessonProgress).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const lmsCertificates = pgTable("lms_certificates", {
  ...
  userId: varchar("user_id").references(() => platformUsers.id).notNull(),
  courseId: varchar("course_id").references(() => lmsCourses.id).notNull(),
  assignmentId: varchar("assignment_id").references(() => lmsCourseAssignments.id),
  ...
  issuedById: varchar("issued_by_id").references(() => platformUsers.id),
  ...
  index("idx_lms_certificates_user").on(table.userId),
  index("idx_lms_certificates_course").on(table.courseId),
]);

export const insertLmsCertificateSchema = createInsertSchema(lmsCertificates).omit({
  id: true,
  createdAt: true,
});
export type InsertLmsCertificate = z.infer<typeof insertLmsCertificateSchema>;
...
// Course Departments - which departments a course targets
export const lmsCourseDepartments = pgTable("lms_course_departments", {
  ...
  unique("lms_course_dept_unique").on(table.courseId, table.department),
]);

export const insertLmsCourseDepartmentSchema = createInsertSchema(lmsCourseDepartments).omit({
  id: true,
  createdAt: true,
});
...
// Course Ratings - user feedback on courses
export const lmsCourseRatings = pgTable("lms_course_ratings", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  courseId: varchar("course_id").references(() => lmsCourses.id, { onDelete: "cascade" }).notNull(),
  userId: varchar("user_id").references(() => platformUsers.id).notNull(),
...
export const insertLmsCourseRatingSchema = createInsertSchema(lmsCourseRatings).omit({
  id: true,
  createdAt: true,
});
export type InsertLmsCourseRating = z.infer<typeof insertLmsCourseRatingSchema>;
...
// Training Portal Sessions - for PIN-based access
export const lmsTrainingPortalSessions = pgTable("lms_training_portal_sessions", {
  ...
  index("idx_lms_portal_sessions_expires").on(table.expiresAt),
]);

export const insertLmsTrainingPortalSessionSchema = createInsertSchema(lmsTrainingPortalSessions).omit({
  id: true,
  createdAt: true,
});
...
// Course Progress Logs - detailed progress events
export const lmsCourseProgressLogs = pgTable("lms_course_progress_logs", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  assignmentId: varchar("assignment_id").references(() => lmsCourseAssignments.id, { onDelete: "cascade" }).notNull(),
...
export const insertLmsCourseProgressLogSchema = createInsertSchema(lmsCourseProgressLogs).omit({
  id: true,
  createdAt: true,
});
export type InsertLmsCourseProgressLog = z.infer<typeof insertLmsCourseProgressLogSchema>;
export type LmsCourseProgressLog = typeof lmsCourseProgressLogs.$inferSelect;
